...
def create_distribution_visualization():
    ...
    for model in models:
        sentiment_counts = {
            ...
            'NEUTRAL': sum(df[model] == 'NEUTRAL') / len(df) * 100,
    ...
    # Plot stacked bars
    for sentiment in sentiments:
        color = '#4CAF50' if sentiment == 'POSITIVE' else '#FFC107' if sentiment == 'NEUTRAL' else '#F44336'
    ...
    # Customize plot
    ax.set_xlim(0, 100)
    ax.set_title('Sentiment Distribution for Three-way Classification by Model (%) without Context', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('three-way_zero_context_sentiment_distribution_by_model.png', dpi=300, bbox_inches='tight')
    plt.close()
